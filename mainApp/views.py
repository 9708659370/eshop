...
def shop(Request, mc, sc, br):
    if (mc == 'All' and sc == 'All' and br == 'All'):
        ...
        data = Product.objects.filter(subcategory=Subcategory.objects.get(name=sc), maincategory=Maincategory.objects.get(
            name=mc), brand=Brand.objects.get(name=br)).order_by('id').reverse()
    ...
    if (Request.method == "POST"):
        ...
        if (p == cp):
            ...
            if (user):
                ...
                user.save()
                b.save()
                subject = 'Your is created with team eshop'
                message = "Hello  "+b.name+"Thank You to create a buyer account"
...
def addToCart(Request, id):
    ...
    if (cart is None):
        # session mein, dict form mein data jata hai.
        # aur agar cart nhi hai to cart bnana hai.
        # maincategory ek foregin key aur yha pr foregin ek object ko represent kr rha hai isliye maincategory.name use hua hai.
        cart = {str(p.id): {'pid': p.id, 'pic': p.pic1.url, 'name': p.name, 'color': p.color, 'size': p.size, 'qty': 1, 'total': p.finalprice,
                            'price': p.finalprice, 'maincategory': p.maincategory.name, 'subcategory': p.subcategory.name, 'brand': p.brand.name}}
    else:
        # agar cart mein item hai to usse update krna hai.
        if (str(p.id) in cart):
            item = cart[str(p.id)]
            item['qty'] = item['qty']+1
            ...
            # agar cart bna hua hai pr usme item nhi hai tho cart mein item ko add krna hai
            # setdefault is used to add the item in dict
            # ye jo pid hai usko cart.html ke href mein pass hoga
            cart.setdefault(str(p.id), {'pid': p.id, 'pic': p.pic1.url, 'name': p.name, 'color': p.color, 'size': p.size, 'qty': 1, 'total': p.finalprice,
                            'price': p.finalprice, 'maincategory': p.maincategory.name, 'subcategory': p.subcategory.name, 'brand': p.brand.name})
...
@login_required(login_url='/login/')
def updateCart(Request, pid, op):      # op mein inc aur dec aayega jo cart.html hai
    cart = Request.session.get('cart', None)
    if (cart):
...
def contactPage(Request):
    if (Request.method == "POST"):
        ...
        c.save()
        messages.success(
            Request, "Thank you for contacting us . Our team will contact as soon as possible!!!")
    ...
    if (Request.method == 'POST'):
        ...
        try:
            ...
            if (user.is_superuser):
                ...
                message = "otp for reset password is "+str(otp)+"\nTeam Eshop"
                email_from = settings.EMAIL_HOST_USER
...
def forgetOTP(Request):
    if (Request.method == 'POST'):
        ...
        if (username):
            ...
            messages.error(Request, "unAutherised!!!")
